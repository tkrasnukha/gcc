...
operands_ok_ldd_std (rtx rt, rtx rt2, rtx rn ATTRIBUTE_UNUSED,
		     HOST_WIDE_INT offset, bool load)
{
  unsigned int t, t2;
  ...
  if (!load
      && !(SMALL_INT_RANGE (offset, (GET_MODE_SIZE (DImode) - 1) & -4,
...
bool
gen_operands_ldd_std (rtx *operands, bool load)
{
  int i, gap;
  ...
  if (operands_ok_ldd_std (operands[0], operands[1], base, offset, false))
    return true;
